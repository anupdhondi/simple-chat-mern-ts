...
wss.on("connection", function connection(ws: CustomWebSocket) {
  ...
  ws.on("close", () => {
    setClients(clients.filter((generalSocket) => generalSocket.connectionID !== ws.connectionID));
  });
...
//how websocket connections works -> When you establish a connection its normal http connection then its upgraded to tcp connection
